...
import { ActionsInicialProps, CustomPaginationActionsTableProps, TableActionsProps, TablePaginationActionsProps } from '../models/interfaces/table';
...
const TablaPaginacionInicial: React.FC<CustomPaginationActionsTableProps> = ({
    columns,
    rows,
    total,
    page,
                ...
                <TableBody>
                    {rows.map((row, index) => (
                        <StyledTableRow key={index}>
                            ...
                            {actions ? (<TableCell component="th" scope="row" align='center'><TableActions actions={actions} id={row.id} order={row.Order} approval={row.Approval} area={row.Area} Description={row.Description} NameUser={row.NameUser} ProjectLeadFinanceID={row.ProjectLeadFinanceID}/></TableCell>) : null}
                        </StyledTableRow>
                    ))}
...
const TableActions: React.FC<ActionsInicialProps> = ({ actions, id, approval, order, area = '', Description, NameUser, ProjectLeadFinanceID }) => {

    const highApprovalOrder = () => {
        return order >= 2;
    }
        ...
        <>
            ...
            >
                {actions.map((prop: TableActionsProps, index: number) => (
                    <div key={index}>
                        {prop.id == 'approve' ? <Button sx={{ background: '#429221' }} onClick={() => prop.function(id, approval, order)}>Aprobar</Button> : null}
                        {prop.id == 'decline' && highApprovalOrder() ? <Button sx={{ background: '#DE1C24' }} onClick={() => prop.function(id, approval, order)}>Cancelar</Button> : null}
                        {prop.id == 'adjust' ? <Button sx={{ background: '#FDBB30' }} onClick={() => prop.function(id, approval, order, area)}>Ajustar datos</Button> : null}
                    </div>
                ))}
            </Stack>
        </>
...
export default TablaPaginacionInicial;
